...
on s.student_id=e.student_id; -- not enrolled students also 

select s.student_id, s.student_name, e.enrollment_id, e.course_name,
e.student_id from students as s left join enrollments as e
on s.student_id=e.student_id where s.student_id=8; -- this is for 8th id

select s.student_id, s.student_name, e.enrollment_id, e.course_name,
e.student_id from students as s left join enrollments as e
on s.student_id=e.student_id where enrollment_id is NULL; -- Here enrollment id is null that students

select s.student_name, count(e.enrollment_id), s.student_id from students as s join enrollments as e
...
select s.student_id, s.student_name, sum(e.credits), count(enrollment_id) from students as s left join enrollments as e
on s.student_id=e.student_id group by student_id 
having count(enrollment_id)>2;

-- each major how many students
...
left join enrollments as e
on s.student_id=e.student_id group by student_id
order by count(enrollment_id) DESC LIMIT 2 OFFSET 1; -- offset = skip ,limit = how many

select s.student_id, s.student_name, char_length count(enrollment_id)
from students as s 
